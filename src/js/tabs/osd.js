...
// Legacy OSD tab expects Betaflight-style API milestones.
// Map them onto current Rotorflight configurator API versions.
const API_VERSION_1_34 = "12.6.0";
const API_VERSION_1_35 = "12.6.0";
const API_VERSION_1_36 = "12.7.0";
const API_VERSION_1_37 = "12.7.0";
const API_VERSION_1_31 = "12.6.0";
const API_VERSION_1_32 = "12.6.0";
const _API_VERSION_1_33 = "12.6.0";
const API_VERSION_1_39 = "12.8.0";
const API_VERSION_1_40 = "12.8.0";
const API_VERSION_1_41 = "12.8.0";
const API_VERSION_1_42 = "12.9.0";
const API_VERSION_1_43 = "12.9.0";
const API_VERSION_1_44 = "12.9.0";
const API_VERSION_1_45 = "12.9.0";
...
function camelize(value) {
    return value
        ...
        .replace(/^(.)/, (_match, chr) => chr.toUpperCase());
}
...
OSD.getDisplayCols = function() {
    return OSD.data.displaySize?.x || FONT.constants.SIZES.LINE;
};
...
OSD.generateCraftName = function() {
    ...
    const craftName = semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_45)
        ? (FC.CONFIG.craftName ?? FC.CONFIG.name ?? '')
        : (FC.CONFIG.name ?? '');
    if (typeof craftName === 'string' && craftName !== '') {
...
OSD.generatePilotName = function() {
  ...
  const pilotName = FC.CONFIG.pilotName ?? FC.CONFIG.displayName ?? FC.CONFIG.name ?? '';
  if (typeof pilotName === 'string' && pilotName !== '') {
      preview = pilotName.toUpperCase();
  }
  return preview;
};
...
OSD.formatPidsPreview = function(axis) {
    const axisPids = FC.PIDS[axis] || [];
    const p = (axisPids[0] || 0).toString().padStart(3);
    const i = (axisPids[1] || 0).toString().padStart(3);
    const d = (axisPids[2] || 0).toString().padStart(3);
    const f = (axisPids[3] || 0).toString().padStart(3);
    return `${p} ${i} ${d} ${f}`;
};
...
OSD.updateDisplaySize = function() {
    ...
    // Keep preview width in sync with active canvas columns (12px glyph width).
    // This is required for HD layouts where width is > 30 columns.
    const previewWidthPx = OSD.data.displaySize.x * 12;
    $('.tab-osd').css('--osd-preview-width', `${previewWidthPx}px`);
};
    ...
    */
        ...
        unpack: {
            position(bits, c) {
                ...
                if (semver.gte(FC.CONFIG.apiVersion, "1.21.0")) {
                    ...
                    displayItem.position = positionable ? OSD.getDisplayCols() * ypos + xpos : defaultPosition;
        ...
        pack: {
            position(displayItem) {
                ...
                if (semver.gte(FC.CONFIG.apiVersion, "1.21.0")) {
                    ...
                    const variantSelected = (displayItem.variant || 0) << 14;
                    const xpos = position % OSD.getDisplayCols();
                    const ypos = (position - xpos) / OSD.getDisplayCols();
        ...
        if (OSD.data.state.haveOsdFeature && semver.gte(FC.CONFIG.apiVersion, "1.21.0")) {
            ...
            if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_37)) {
                ...
                if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_47)) {
                    const rssiDbm = OSD.data.alarms.rssi_dbm?.value ?? 0;
                    result.push16((rssiDbm + 0x10000) & 0xFFFF);
                }
            }
        }
        return result;
    ...
    decode(payload) {
        ...
        while (view.offset < view.byteLength && itemsPositionsRead.length < displayItemsCountActual) {
            const v = semver.gte(FC.CONFIG.apiVersion, "1.21.0") ? view.readU16() : view.read16();
...
OSD.GUI.preview = {
    ...
    onDrop(e) {
        ...
        const cursorX = cursor % OSD.getDisplayCols();
        ...
        if (!displayItem.ignoreSize) {
            if (displayItem.preview.constructor !== Array) {
                ...
                const overflowsLine = OSD.getDisplayCols() - ((position % OSD.getDisplayCols()) + displayItem.preview.length);
                if (overflowsLine < 0) {
                ...
                const arrayElements = displayItem.preview;
                const limits = OSD.searchLimitsElement(arrayElements);
                const selectedPositionX = position % OSD.getDisplayCols();
                let selectedPositionY = Math.trunc(position / OSD.getDisplayCols());
                if (arrayElements[0].constructor === String) {
                    if (position < 0 ) {
                        return;
                    }
                    if (selectedPositionX > cursorX) { // TRUE -> Detected wrap around
                        position += OSD.getDisplayCols() - selectedPositionX;
                        selectedPositionY++;
                    } else if (selectedPositionX + limits.maxX > OSD.getDisplayCols()) { // TRUE -> right border of the element went beyond left edge of screen.
                        position -= selectedPositionX + limits.maxX - OSD.getDisplayCols();
                    }
                    if (selectedPositionY < 0 ) {
                        position += Math.abs(selectedPositionY) * OSD.getDisplayCols();
                    } else if ((selectedPositionY + limits.maxY ) > OSD.data.displaySize.y) {
                        position -= (selectedPositionY + limits.maxY  - OSD.data.displaySize.y) * OSD.getDisplayCols();
                    }

                } else {
                    if ((limits.minX < 0) && ((selectedPositionX + limits.minX) < 0)) {
                        position += Math.abs(selectedPositionX + limits.minX);
                    } else if ((limits.maxX > 0) && ((selectedPositionX + limits.maxX) >= OSD.getDisplayCols())) {
                        position -= (selectedPositionX + limits.maxX + 1) - OSD.getDisplayCols();
                    }
                    if ((limits.minY < 0) && ((selectedPositionY + limits.minY) < 0)) {
                        position += Math.abs(selectedPositionY + limits.minY) * OSD.getDisplayCols();
                    } else if ((limits.maxY > 0) && ((selectedPositionY + limits.maxY) >= OSD.data.displaySize.y)) {
                        position -= (selectedPositionY + limits.maxY - OSD.data.displaySize.y + 1) * OSD.getDisplayCols();
                    }
                }
            }
        }
...
TABS.osd.initialize = function(callback) {
    ...
    $('#content').load("/src/tabs/osd.html", function() {
        ...
        function updateOsdView() {
            $('.noOsdChipDetect, .unsupported, .supported').hide();
            // Ask FC for HD canvas first when available; ignore unsupported message fallback.
            ...
            fetchCanvas.then(() => MSP.promise(MSPCodes.MSP_OSD_CONFIG))
                .then(function(info) {
                    ...
                    const hasBuildOptionGating = semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_45) && buildOptions.length > 0;
                    for (let i = 0; i < OSD.constants.VIDEO_TYPES.length; i++) {
                        const type = OSD.constants.VIDEO_TYPES[i];
                        const videoFormatKey = `osdSetupVideoFormatOption${camelize(type.toLowerCase())}`;
                        const translatedVideoFormat = i18n.getMessage(videoFormatKey);
                        const videoFormatOptionText = translatedVideoFormat && translatedVideoFormat !== videoFormatKey ? translatedVideoFormat : type;
                        const disabled = hasBuildOptionGating &&
                            ((type !== 'HD' && !buildOptions.includes('USE_OSD_SD')) ||
                                (type === 'HD' && !buildOptions.includes('USE_OSD_HD')));
                        ...
                        $checkbox.find(':radio').prop('disabled', disabled);
                        $videoTypes.append($checkbox);
                    }
                    $videoTypes.find(':radio').change(function() {
                        if ($(this).is(':disabled')) {
                            return;
                        }
                    ...
                    if (semver.gte(FC.CONFIG.apiVersion, "1.21.0")) {
                        ...
                        for (let i = 0; i < OSD.constants.UNIT_TYPES.length; i++) {
                            ...
                            const setupUnitOptionText = i18n.getMessage(`osdSetupUnitsOption${camelize(type.toLowerCase())}`);
                            const $checkbox = $('<label/>')
                        ...
                        if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_36)) {
                            ...
                            for (const tim of OSD.data.timers) {
                                ...
                                OSD.constants.TIMER_TYPES.forEach(function(e, i) {
                                    const timerSourceOptionText = i18n.getMessage(`osdTimerSourceOption${camelize(e.toLowerCase())}`);
                                ...
                                OSD.constants.TIMER_PRECISION.forEach(function(e, i) {
                                    const timerPrecisionOptionText = i18n.getMessage(`osdTimerPrecisionOption${camelize(e.toLowerCase())}`);
                    ...
                    // Keep video format selector visible for OSD-enabled targets even if
                    // legacy MAX7456/MSP device flags are not exposed by older firmware.
                    if (OSD.data.state.haveOsdFeature) {
                        $('.videomode-container').show();
                        ...
                        if (field.preview.constructor !== Array) {
                            ...
                            for (let i = 0; i < arrayElements.length; i++) {
                                ...
                                if (element.constructor === String) {
                                    ...
                                    selectedPosition = selectedPosition - element.length + OSD.getDisplayCols();
                                } else {
                                    const limits = OSD.searchLimitsElement(arrayElements);
                                    let offsetX = 0;
                                    let offsetY = 0;
                                        ...
                                        OSD.drawByOrder(selectedPosition + element.x + element.y * OSD.getDisplayCols(), field, charCode, element.x, element.y);
                                        // Image used when "dragging" the element
                                        if (field.positionable) {
                                            const img = new Image();
                                            img.src = FONT.draw(charCode);
